...
void KmerIndex::BuildDistinguishingGraph(const ProgramOptions& opt, std::ofstream& out) {
  ...
  std::cerr << "[build] k-mer length: " << k << std::endl;
  size_t ncolors = 0;
  bool generateColoredDBG = !opt.distinguishUseInput;
  std::string tmp_file2 = opt.distinguishFile.empty() ? generate_tmp_file("--" + opt.index) : opt.distinguishFile;
  if (generateColoredDBG) { // Start generate colored DBG
  ...
  if (opt.distinguishUseInput) { // Use an external input FASTA file (we'll still need to read it to determine number of targets though and we'll still write out a new FASTA file (TODO: optimize this out)
    ...
    int l = 0;
    size_t num_seqs = 0;
    size_t num_d_seqs = 0; // D-list sequences (that will not be included)
    ...
    for (int i = 0; i < opt.transfasta.size(); i++) { // Currently, this should only be one file
      ...
      while (true) {
        ...
        bool dlisted_color = strname[0] == 'd'; // D-listed color: d_list.
        if (dlisted_color) {
          num_d_seqs++;
          continue;
        }
    ...
    if (num_d_seqs > 0) {
      std::cerr << "[build] warning: Discarded " << num_d_seqs << " distinguishing flanking k-mer sequences" << std::endl;
    }
  ...
  if (removeGeneratedFile) std::remove(tmp_file2.c_str());
