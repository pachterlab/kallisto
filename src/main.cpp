...
void ListSingleCellTechnologies()
{
  ...
  cout << "List of supported single-cell technologies" << endl
       << endl
...
void ParseOptionsBus(int argc, char **argv, ProgramOptions &opt)
{
  ...
  const char *opt_string = "i:o:x:t:lbnkg:c:";
  static struct option long_options[] = {
      {"verbose", no_argument, &verbose_flag, 1},
      {"index", required_argument, 0, 'i'},
      {"output-dir", required_argument, 0, 'o'},
...
bool CheckOptionsBus(ProgramOptions &opt)
{
  ...
  else
  {
    ...
    if (opt.bam)
    { // Note: only 10xV2 has been tested
      busopt.nfiles = 1;
...
bool CheckOptionsEM(ProgramOptions &opt, bool emonly = false)
{
  ...
  {
    cerr << "[quant] fragment length distribution is truncated gaussian with mean = " << opt.fld << ", sd = " << opt.sd << endl;
  }
...
void PrintCite()
{
  cout << "When using this program in your research, please cite" << endl
       << endl
...
void usage()
{
  cout << "kallisto " << KALLISTO_VERSION << endl
       << endl
       << "Usage: kallisto <CMD> [arguments] .." << endl
       << endl
       << "Where <CMD> can be one of:" << endl
       << endl
       ...
       << endl
       << "Running kallisto <CMD> without arguments prints usage information for <CMD>" << endl
       << endl;
}
...
{
  cout << "kallisto " << KALLISTO_VERSION << endl
       << "Generates BUS files for single-cell sequencing" << endl
       << endl
       << "Usage: kallisto bus [arguments] FASTQ-files" << endl
       << endl
       ...
       << "-k, --kmer                    Alignment is done per kmer" << endl
...
{
  cout << "kallisto " << KALLISTO_VERSION << endl
       << "Builds a kallisto index" << endl
       << endl
       << "Usage: kallisto index [arguments] FASTA-files" << endl
       << endl
       << "Required argument:" << endl
       << "-i, --index=STRING          Filename for the kallisto index to be constructed " << endl
       << endl
...
{
  cout << "kallisto " << KALLISTO_VERSION << endl
       << "Converts HDF5-formatted results to plaintext" << endl
       << endl
       << "Usage:  kallisto h5dump [arguments] abundance.h5" << endl
       << endl
       << "Required argument:" << endl
       << "-o, --output-dir=STRING       Directory to write output to" << endl
       << endl;
...
void usageInspect()
{
  cout << "kallisto " << KALLISTO_VERSION << endl
       << endl
       << "Usage: kallisto inspect INDEX-file" << endl
       << endl
       << "Optional arguments:" << endl
       << "-G, --gfa=STRING        Filename for GFA output of T-DBG" << endl
       << "-g, --gtf=STRING        Filename for GTF file" << endl
       << "-b, --bed=STRING        Filename for BED output (default: index + \".bed\")" << endl
       << endl;
}
...
void usageEM(bool valid_input = true)
{
  if (valid_input)
  {
    ...
    cout << "kallisto " << KALLISTO_VERSION << endl
         << "Computes equivalence classes for reads and quantifies abundances" << endl
         << endl;
  }
  //      "----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|----|"
  cout << "Usage: kallisto quant [arguments] FASTQ-files" << endl
       << endl
       << "Required arguments:" << endl
       << "-i, --index=STRING            Filename for the kallisto index to be used for" << endl
       << "                              quantification" << endl
...
void usagePseudo(bool valid_input = true)
{
  ...
  cout << "Usage: kallisto pseudo [arguments] FASTQ-files" << endl
       << endl
       << "Required arguments:" << endl
       << "-i, --index=STRING            Filename for the kallisto index to be used for" << endl
       << "                              pseudoalignment" << endl
...
void usageMerge(bool valid_input = true)
{
  ...
  cout << "Usage: kallisto merge [arguments] ouput-directories" << endl
       << endl
       << "Required arguments:" << endl
       << "-i, --index=STRING            Filename for the kallisto index to be used for" << endl
       << "                              pseudoalignment" << endl
...
{
  cout << "kallisto " << KALLISTO_VERSION << endl
       << "Computes equivalence classes for reads and quantifies abundance" << endl
       << endl
       << "Usage: kallisto quant-only [arguments]" << endl
       << endl
       << "Required argument:" << endl
       << "-o, --output-dir=STRING       Directory to write output to" << endl
       << endl
      ...
      else
      {
        ...
        else if (opt.bootstrap > 0 && num_pseudoaligned > 0)
        {
          ...
          if (opt.threads > 1)
          {
            ...
            {
              cerr
                  << "[~warn] number of threads (" << opt.threads << ") greater than number of bootstraps" << endl
      ...
      else
      {
        ...
        else if (opt.bootstrap > 0 && num_pseudoaligned > 0)
        {
          ...
          if (opt.threads > 1)
          {
            ...
            if (opt.threads > opt.bootstrap)
            {
              cerr << "[btstrp] Warning: number of threads (" << opt.threads << ") greater than number of bootstraps." << endl
    ...
    else if (cmd == "pseudo")
    {
      ...
      else
      {
        ...
        if (opt.pseudo_quant)
        {
          ...
          std::cerr << "[quant] Running EM algorithm for each cell ..";
          std::cerr.flush();
