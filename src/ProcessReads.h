...
{
public:
  SequenceReader(const ProgramOptions &opt) : readbatch_id(-1){};
...
{
public:
  BamSequenceReader(const ProgramOptions &opt) : SequenceReader(opt)
  {
...
public:
  MasterProcessor(KmerIndex &index, const ProgramOptions &opt, MinCollector &tc, const Transcriptome &model)
      : tc(tc), index(index), model(model), bamfp(nullptr), bamfps(nullptr), bamh(nullptr), opt(opt), numreads(0), nummapped(0), num_umi(0), bufsize(1ULL << 23), tlencount(0), biasCount(0), maxBiasCount((opt.bias) ? 1000000 : 0), last_pseudobatch_id(-1)
  {
    if (opt.bam)
    {
