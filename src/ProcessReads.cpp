...
int64_t ProcessReads(MasterProcessor &MP, const ProgramOptions &opt)
{
  ...
  {
    std::cerr << "[quant] processed " << pretty_num(numreads) << " reads, "
              << pretty_num(nummapped) << " kmers pseudoaligned";
  }
  else
  {
    std::cerr << "[quant] processed " << pretty_num(numreads) << " reads, "
...
ReadProcessor::ReadProcessor(const KmerIndex &index, const ProgramOptions &opt, const MinCollector &tc, MasterProcessor &mp, int _id, int _local_id) : paired(!opt.single_end), tc(tc), index(index), mp(mp), id(_id), local_id(_local_id)
{
  // initialize buffer
  bufsize = mp.bufsize;
...
BUSProcessor::BUSProcessor(const KmerIndex &index, const ProgramOptions &opt, const MinCollector &tc, MasterProcessor &mp, int _id, int _local_id) : paired(!opt.single_end), bam(opt.bam), num(opt.num), tc(tc), index(index), mp(mp), id(_id), local_id(_local_id), numreads(0)
{
  // initialize buffer
  bufsize = mp.bufsize;
  buffer = new char[bufsize];
...
BUSProcessor::BUSProcessor(BUSProcessor &&o) : paired(o.paired),
                                               bam(o.bam),
                                               num(o.num),
                                               kmer(o.kmer),
                                               tc(o.tc),
...
void BUSProcessor::processBuffer()
{
  ...
  for (int i = 0; i + incf < seqs.size(); i++)
  {
    ...
    */
    ...
    // want to make multiple bus records for each kmer that algns
    if (mp.opt.kmer)
    {
...
FastqSequenceReader::FastqSequenceReader(FastqSequenceReader &&o) : nfiles(o.nfiles),
                                                                    numreads(o.numreads),
                                                                    fp(std::move(o.fp)),
                                                                    l(std::move(o.l)),
                                                                    nl(std::move(o.nl)),
